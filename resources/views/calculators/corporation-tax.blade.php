...
function addExpenseRow(type) {
    ...
    expenseRow.className = 'flex gap-2 items-center';
    expenseRow.innerHTML = `
        <input type="text" 
               placeholder="Expense name" 
               class="flex-1 px-3 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all text-sm">
        <input type="number" 
               placeholder="Amount" 
               step="0.01"
               min="0"
               class="w-32 px-3 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all text-sm expense-amount"
               data-type="${type}">
        <button type="button" 
...
function addExpenseRowWithName(type, name) {
    const containerId = type === 'allowed' ? 'allowedExpenses' : 'addBackExpenses';
    const container = document.getElementById(containerId);
    const expenseId = `expense_${type}_${expenseIdCounter++}`;
    ...
    expenseRow.className = 'flex gap-2 items-center';
    expenseRow.innerHTML = `
        <label class="flex-1 px-3 py-2 text-sm text-gray-700 font-medium">
            ${name}
        </label>
        <input type="number" 
               placeholder="Amount" 
...
document.addEventListener('DOMContentLoaded', function() {
    // Add default allowed expense rows with pre-filled names
    addExpenseRowWithName('allowed', 'Salary');
    addExpenseRowWithName('allowed', 'Travel');
    
    // Add default add back expense rows with pre-filled names
    addExpenseRowWithName('addback', 'Business Entertainment');
    addExpenseRowWithName('addback', 'Depreciation');
    addExpenseRowWithName('addback', 'Repayment of Loan');
});
</script>
@endverbatim
@endsection
